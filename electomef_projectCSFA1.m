...
% Example2: TrainCSFA('data/dataStore.mat','data/Mhold.mat',[],[],'data/chkpt_81LNf_Mhold.mat')
  ...
  sets = loadSets1(saveFile,loadFile,nWin); % modified by Yuichi 190724
%  load(saveFile, 'sets') % appended, by Yuichi 190724
  ...
  modelOpts.W = sum(sets.test);    % # of windows
...
%     initScores(:,sets.test) = trainModels(k).scores;
%   end
    initScores = zeros(modelOpts.L,nWin); % all channels are fine
...
function sets = loadSets1(saveFile,loadFile,nWin)
